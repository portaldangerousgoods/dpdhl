(function () {
  ...
  function hasAuth(){
    try{const t=JSON.parse(localStorage.getItem('dg_auth')||'{}');return t.ok&&Date.now()<t.exp;}catch{return false;}
  }
  ...
  /* Botão SAIR (renderiza sempre que estiver na página principal) */
  function injectLogout(){
    ...
    Object.assign(a.style,{
      marginLeft:'16px', marginRight:'40px', padding:'10px 28px', borderRadius:'6px',
      background:'#D40511', color:'#fff', fontWeight:'700', fontSize:'15px', lineHeight:'1.3',
      cursor:'pointer', boxShadow:'0 3px 6px rgba(0,0,0,.15)', transition:'all .2s ease-in-out'
    });
  ...
  /* Esconde link GitHub no header (se existir) */
  ...
  function injectTopNavMega(){
    ...
    if(drop && trigger && panel && drop.dataset.wired!=='1'){
      ...
      trigger.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();drop.classList.toggle('open');trigger.setAttribute('aria-expanded',drop.classList.
...
contains('open')?'true':'false');});
  ...
  /* Força o dropdown de resultados do Material a aparecer sob a barra */
  function wireSearchDropdown(){
    ...
    const result=root.querySelector('[data-md-component="search-result"]')||root.querySelector('.md-search__output');
    const overlay=root.querySelector('.md-search__overlay');
    if(overlay) overlay.style.display='none';
    root.style.position='absolute';
    if(result){ result.style.position='absolute'; result.style.left='0'; result.style.top='calc(100% + 8px)'; }

    function openDD(){ if(!result) return; root.setAttribute('data-md-state','active'); result.removeAttribute('hidden'); }
    function closeDD(){ if(!result) return; result.setAttribute('hidden',''); root.removeAttribute('data-md-state'); }
